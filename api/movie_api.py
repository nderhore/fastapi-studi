...
class MovieAPI:
    ...
    def add_routes(self):
        ...
        def create_movie_endpoint(movie: MovieCreate, request: Request, db: Session = Depends(get_db)):
        ...
        def update_movie_endpoint(movie_id: int, updated_movie: MovieCreate, request: Request, db: Session = Depends(get_db)):
            verify_token(request)
            return update_movie(movie_id, updated_movie, db)

        @self.router.delete("/{movie_id}", response_model=MovieResponse)
        def delete_movie_endpoint(movie_id: int, request: Request, db: Session = Depends(get_db)):
            verify_token(request)
